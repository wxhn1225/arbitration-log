...
- **特殊处理**：LoopDefend.lua 的内部计数器 buffer 长度为 29，超过后计数器归零（如 29→1）。归零时的 `wave 1` 日志是脚本内部重置事件，不是真实波次。解析器检测到归零时将 offset 设为 `lastWave − 1`，跳过该归零标记，确保后续波次编号正确累计。
